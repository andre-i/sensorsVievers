class DeviceWorker{#gauge;constructor(){}checkAndSet(gauge,gProps){if(!gauge||!gProps)throw Error("Can`t create DevicePanel. Goten values is wrong")}createPanel(gauge,gProps,owner,controlButton){this.checkAndSet(gauge,gProps);this.#gauge=gauge;let labels=gProps.labels;let scales=gProps.scales;let panel=document.createElement("div");panel.setAttribute("class","devPanel");let device={isVisible:false,owner:owner,cButton:controlButton,gauge:gauge,scales:scales,panel:panel};let hMenu=this.makeMenu(device,labels);panel.appendChild(hMenu);let line=document.createElement("hr");panel.appendChild(line);panel.appendChild(gauge.options.renderTo);gauge.draw();controlButton.onclick=this.toggleVisibility.bind(device);return device}makeMenu(device,labels){let menu=document.createElement("div");menu.setAttribute("class","scalesMenu");let i=0;for(let label of labels){let b=document.createElement("button");b.setAttribute("class","devButton");b.onclick=this.clickOnButton.bind(device,i);b.innerText=label;menu.appendChild(b);i++}return menu}setValue(newValue){if(Number.isFinite(newValue)&&newValue>-50)gauge.update({value:newValue});else{document.getElementById("errPanel").innerText=" Ошибка при получении данных с устройства [ "+newValue+" ]"+"<br> Возможно устройство не подключено или неисправно. Попробуйте обновить список.";this.toggleVisibility()}}clickOnButton(n){let scale=this.scales[n];let dashLabels=[];let shift=(scale.max-scale.min)/scale.count;let i=0;dashLabels[0]=scale.min;for(i=1;i<=scale.count;i++)dashLabels[i]=Math.floor((scale.min+shift*i)*10)/10;let shorDashCount=scale.count>5?5:10;this.gauge.highlights=[{from:scale.min,to:scale.max,color:"rgb(255, 255, 255)"}];this.gauge.resetScale(scale.min,scale.max,dashLabels,shorDashCount)}toggleVisibility(){if(!this.cButton.classList.contains("button-enabled")&&!this.cButton.classList.contains("button-enabled-active"))return;if(this.isVisible){this.isVisible=false;this.owner.removeChild(this.panel);if(this.cButton.classList.contains("button-enabled-active"))this.cButton.classList.toggle("button-enabled-active")}else{this.isVisible=true;this.owner.appendChild(this.panel);this.panel.setAttribute("style","width :"+(this.gauge.options.width+10)+"px");this.cButton.classList.toggle("button-enabled-active")}}}
...
class DevicesController{#controls;#gauges;#activeDev={};#modules={};#owner;#intervalId=0;#delay=2e4;constructor(devicesButtons,deviceOwner){this.#controls=devicesButtons;this.#gauges=new ModulesGauges;this.#modules=new MyModules;this.#owner=deviceOwner}showActiveDevices(){let d="";for(let name in this.#activeDev)d+=name+" : ";console.log("Connected sensors "+d)}addDevices(sensorName){if(!sensorName)return;let devWorker=new DeviceWorker;let sensor=this.#modules.getSensor(sensorName);let devNames=sensor.devices;let controls=this.#controls[sensorName];for(let name of devNames){let props=sensor.device[name];let gauge=this.#gauges.getGauge(props.gName);let device=devWorker.createPanel(gauge,props,this.#owner,controls[name]);this.#activeDev[props.gName]=device}if(this.#delay>sensor.delayMs)this.#delay=sensor.delayMs}startWatch(){let myHandler=this.getValuesHandler(this.#activeDev);let delay=this.#delay;let h=this.handleRequest.bind(this,"/values",myHandler.bind(this));this.#intervalId=setInterval(h,delay)}getValuesHandler(devicesHandlers){let myHandler=function(values,isError){if(isError){this.showError(values);return}let res;try{res=JSON.parse(values)}catch(err){res={e:""+values+"<br> ошибка [ "+err+" ]"}}for(let sensor in res){let dig=res[sensor];if(!dig&&dig!=0)return;if(Number.isFinite(dig)&&dig>-60){dig=Math.floor(dig*10)/10;if(sensor=="BMP280_P")dig=Math.floor(dig)/1e3;devicesHandlers[sensor].gauge.update({value:dig})}else{this.showError("Неверные данные - "+dig)}}};return myHandler}clear(){this.#owner.innerText="";let but=document.getElementsByClassName("valButton");for(let b of but){if(b.classList.contains("button-enabled-active"))b.classList.toggle("button-enabled-active")}clearInterval(this.#intervalId);for(let dev in this.#activeDev)this.#activeDev[dev]=null;this.#activeDev={};this.#delay=2e4;document.getElementById("errPanel").innerHTML=""}handleRequest(request,handler){let xhttp=new XMLHttpRequest;try{xhttp.onreadystatechange=function(){if(this.readyState==4){let answer=this.responseText;if(this.status==200){handler(answer,false);return}if(this.status==0){handler(" ESP8266 не отвечает ! ",true);return}else if(this.status>=300){if(answer==undefined||answer==""){answer=" Проблема на сервере статус ответа <u>"+this.status+"</u>"}console.log("ERROR from server on load data [ "+answer+" ] status="+this.status);handler(answer,true)}}};xhttp.open("GET",request,true);xhttp.send()}catch(error){console.log("catched error: "+e);if(error){document.getElementById("errPanel").innerText=error}}}init(){this.clear();for(let d in this.#controls){let s=this.#controls[d];for(let b in s){let el=s[b];if(el.classList.contains("button-enabled"))el.classList.toggle("button-enabled");if(el.classList.contains("button-enabled-active"))el.classList.toggle("button-enabled-active")}}function initHandler(answer,err){if(err){this.showError(answer)}else{let sArray=answer.trim().split(":");for(let sName of sArray){let sensor=this.#controls[sName];for(let b in sensor){sensor[b].classList.toggle("button-enabled")}this.addDevices(sName)}this.showActiveDevices();document.getElementById("errPanel").innerText="";this.startWatch()}}let h=initHandler.bind(this);this.handleRequest("/list",h)}showError(answer){this.clear();document.getElementById("errPanel").innerHTML="<h2>Проблемы с чтением значений</h2>"+" <div><big><b>"+answer+"</b></big></div><h2>Проверьте состояние устройства(ESP8266) и попробуйте обновить список модулей</h2>"}}
let modulesNames=["AHT20","DS18B20","INA219","BMP280"];class MyModules{constructor(){}getSensor(sensorName){return this.#modulesProperties[sensorName]}#modulesProperties={AHT20:{delayMs:1e4,devices:["Fi","t"],device:{Fi:{gName:"AHT20_H",scales:[{max:100,min:0,count:10},{max:100,min:50,count:5},{max:50,min:0,count:5}],labels:["100","50:100","50"]},t:{gName:"AHT20_T",scales:[{max:80,min:-40,count:6},{max:50,min:0,count:5},{max:0,min:-40,count:4}],labels:["-40:80","0:50","-40:0"]}}},DS18B20:{delayMs:2e3,devices:["t"],device:{t:{gName:"DS18B20",scales:[{max:10,min:-50,count:6},{max:50,min:0,count:5},{max:110,min:50,count:6},{max:125,min:75,count:5},{max:120,min:-20,count:8}],labels:["-50:10","0:50","50:110","75:125","-20:120"]}}},INA219:{delayMs:450,devices:["U","I","P"],device:{I:{gName:"INA219_I",scales:[{max:1,min:0,count:5},{max:2,min:0,count:4},{max:4,min:1,count:6},{max:5,min:0,count:5}],labels:["0:1","0:2","1:4","0:5"]},U:{gName:"INA219_U",scales:[{max:5,min:0,count:5},{max:10,min:0,count:5},{max:25,min:0,count:5},{max:15,min:5,count:5}],labels:["5","10","25","5:15"]},P:{gName:"INA219_P",scales:[{max:5,min:0,count:5},{max:10,min:0,count:5},{max:50,min:0,count:5},{max:150,min:0,count:6}],labels:["5","10","50","150"]}}},BMP280:{delayMs:1100,devices:["P","t"],device:{P:{gName:"BMP280_P",scales:[{max:100,min:90,count:5},{max:105,min:95,count:5},{max:102,min:98,count:4}],labels:["90:100","95:105","98:102"]},t:{gName:"BMP280_T",scales:[{max:0,min:-40,count:4},{max:50,min:0,count:5},{max:50,min:-50,count:10}],labels:["-40:0","0:50"]}}}}}
let g_height=3.2*document.documentElement.clientHeight/4;class ModulesGauges{constructor(){}ds_t=new RadialGauge({renderTo:document.createElement("canvas"),width:g_height,height:g_height,title:"t (DS18B20)",units:"°C",fontTitleSize:40,fontUnitsSize:40,fontNumbersSize:35,colorStrokeTicks:"#002",colorNumbers:"#200",colorTitle:"rgb(1, 1 ,50)",colorUnits:"rgb(20, 30, 1)",valueBox:false,minValue:-20,maxValue:120,value:0,majorTicks:[-20,0,20,40,60,80,100,120],minorTicks:10,strokeTicks:true,colorPlate:"#cfc",colorValueBoxRect:"fff",borderShadowWidth:10,borders:false,borderOuterWidth:20,needleType:"arrow",needleWidth:5,needleCircleSize:0,needleCircleOuter:false,needleCircleInner:false,colorNeedle:"#020",colorNeedleEnd:"#030",animationDuration:1500,animationRule:"linear",barProgress:false,highlights:[{from:-20,to:120,color:"#fff"}]});aht_t=new RadialGauge({renderTo:document.createElement("canvas"),width:g_height,height:g_height,title:"t (AHT)",units:"°C",fontTitleSize:40,fontUnitsSize:40,fontNumbersSize:35,strokeTicks:true,colorStrokeTicks:"#fff",colorMinorTicks:"#fff",colorNumbers:"#318",colorPlate:"#ccf",colorPlateEnd:"#99c",colorTitle:"rgb(2, 1 ,6)",colorUnits:"rgb(8, 1, 1)",valueBox:false,minValue:-50,maxValue:50,majorTicks:[-50,-40,-30,-20,-10,0,10,20,30,40,50],minorTicks:10,borders:true,borderOuterWidth:10,needleType:"arrow",needleWidth:5,needleCircleSize:7,needleCircleOuter:true,needleCircleInner:false,colorNeedle:"#942",colorNeedleEnd:"#ffe",animationDuration:1500,animationRule:"linear",barProgress:false,highlights:[{from:-50,to:50,color:"#219"}]});aht_hum=new RadialGauge({renderTo:document.createElement("canvas"),width:g_height,height:g_height,title:"Влажность",units:"%",fontTitleSize:40,fontUnitsSize:40,fontNumbersSize:35,colorStrokeTicks:"#002",colorNumbers:"#200",colorTitle:"rgb(2, 1 ,6)",colorUnits:"rgb(8, 1, 1)",colorPlate:"#88f",colorPlateEnd:"#cfa",valueBox:false,minValue:0,maxValue:100,majorTicks:[0,20,40,60,80,100],minorTicks:10,strokeTicks:true,borders:false,borderOuterWidth:20,needleType:"arrow",needleWidth:3,needleCircleSize:7,needleCircleOuter:true,needleCircleInner:false,colorNeedle:"#228",colorNeedleEnd:"#035",animationDuration:1500,animationRule:"linear",barProgress:false,highlights:[{from:0,to:100,color:"#fff"}]});ina_u=new RadialGauge({renderTo:document.createElement("canvas"),width:g_height,height:g_height,title:"Напряжение",units:"В",fontTitleSize:40,fontUnitsSize:40,fontNumbersSize:35,strokeTicks:true,colorStrokeTicks:"#fff",colorMinorTicks:"#fff",colorNumbers:"#fff",colorTitle:"rgb(2, 1 ,6)",colorUnits:"rgb(8, 1, 1)",colorPlate:"#b97",colorPlateEnd:"#964",valueBox:false,minValue:0,maxValue:25,majorTicks:[0,5,10,15,20,25],minorTicks:10,borders:false,borderOuterWidth:20,needleType:"line",needleStart:"12",needleWidth:2,needleCircleOuter:false,needeleCircleInner:false,colorNeedle:"#321",colorNeedleEnd:"#321",animationDuration:1500,animationRule:"linear",barProgress:false,highlights:[{from:0,to:100,color:"#742"}]});ina_i=new RadialGauge({renderTo:document.createElement("canvas"),width:g_height,height:g_height,title:"Сила тока ",units:"A",fontTitleSize:40,fontUnitsSize:40,fontNumbersSize:35,strokeTicks:true,colorStrokeTicks:"#001",colorNumbers:"#200",colorTitle:"rgb(2, 1 ,6)",colorUnits:"rgb(8, 1, 1)",colorPlate:"#598",colorPlateEnd:"#cfe",valueBox:false,minValue:0,maxValue:5,majorTicks:[0,1,2,3,4,5],minorTicks:10,borders:false,borderOuterWidth:20,needleType:"arrow",needleWidth:5,needleCircleSize:4,needleCircleOuter:true,colorNeedle:"#111",colorNeedleEnd:"#111",animationDuration:1500,animationRule:"linear",barProgress:false,highlights:[{from:0,to:100,color:"#fff"}]});ina_p=new RadialGauge({renderTo:document.createElement("canvas"),width:g_height,height:g_height/1,title:"Мощность",units:"Вт",fontTitleSize:40,fontUnitsSize:40,fontNumbersSize:35,colorStrokeTicks:"#002",colorNumbers:"#200",colorTitle:"rgb(2, 1 ,6)",colorUnits:"rgb(8, 1, 1, 1)",startAngle:70,ticksAngle:220,valueBox:false,minValue:0,maxValue:150,majorTicks:[0,25,50,75,100,125,150],minorTicks:10,strokeTicks:true,colorPlate:"#ff8",borderShadowWidth:0,borders:false,borderOuterWidth:20,needleType:"arrow",needleWidth:4,needleCircleSize:7,needleCircleOuter:true,needleCircleInner:false,colorNeedle:"#f11",colorNeedleEnd:"#006",animationDuration:500,animationRule:"linear",barProgress:false,highlights:[{from:0,to:250,color:"#fff"}]});bmp_t=new RadialGauge({renderTo:document.createElement("canvas"),width:g_height,height:g_height,title:"t (BMP280)",units:"°C",fontTitleSize:40,fontUnitsSize:40,fontNumbersSize:35,strokeTicks:true,colorStrokeTicks:"#fff",colorMinorTicks:"#fff",colorNumbers:"#aff",colorPlate:"#135",colorPlateEnd:"#79b",colorTitle:"#ffe",colorUnits:"#ffe",valueBox:false,minValue:-50,maxValue:50,majorTicks:[-50,-40,-30,-20,-10,0,10,20,30,40,50],minorTicks:10,borders:true,borderOuterWidth:10,needleType:"arrow",needleWidth:5,needleCircleSize:7,needleCircleOuter:true,needleCircleInner:false,colorNeedle:"#741",colorNeedleEnd:"#ffe",animationDuration:1500,animationRule:"linear",barProgress:false,highlights:[{from:-50,to:50,color:"#136"}]});bmp_pr=new RadialGauge({renderTo:document.createElement("canvas"),width:g_height,height:g_height/1,title:"Давление",units:"кПа",fontTitleSize:40,fontUnitsSize:40,fontNumbersSize:30,colorStrokeTicks:"#002",colorNumbers:"#200",colorTitle:"rgb(2, 1 ,6)",colorUnits:"rgb(8, 1, 1, 1)",startAngle:70,ticksAngle:220,valueBox:false,minValue:95,maxValue:105,majorTicks:[95,97,99,101,103,105],minorTicks:10,strokeTicks:true,colorPlate:"#cce",borderShadowWidth:0,borders:false,borderOuterWidth:20,needleType:"arrow",needleWidth:5,needleCircleSize:8,needleCircleOuter:true,needleCircleInner:false,colorNeedle:"#333",colorNeedleEnd:"#550",animationDuration:500,animationRule:"linear",barProgress:false,highlights:[{from:-50,to:50,color:"#fff"}]});#all={DS18B20:this.ds_t,AHT20_T:this.aht_t,AHT20_H:this.aht_hum,INA219_U:this.ina_u,INA219_I:this.ina_i,INA219_P:this.ina_p,BMP280_T:this.bmp_t,BMP280_P:this.bmp_pr};getGauge(gaugeName){let g=this.#all[gaugeName];g.resetScale=function(minVal,maxVal,mTicks,tickCount){g.update({minValue:minVal,maxValue:maxVal,majorTicks:mTicks,minorTicks:tickCount})};g.options.renderTo.addEventListener("click",()=>{this.makeDialog(g)});return g}makeDialog(gauge){let intervalId=0;let h=document.documentElement.clientHeight-20;let dialog=document.createElement("dialog");let label=document.createElement("div");label.setAttribute("class","hugePanelLabel");label.innerHTML=gauge.options.title;dialog.appendChild(label);let valPanel=document.createElement("div");if(navigator.userAgent.indexOf("Chrom")>-1)valPanel.setAttribute("class","chrom_digitPanel");else valPanel.setAttribute("class","digitPanel");valPanel.innerText=gauge.value;dialog.appendChild(valPanel);document.body.appendChild(dialog);dialog.addEventListener("click",()=>{clearInterval(intervalId);dialog.close()});intervalId=setInterval(function(){valPanel.innerText=gauge.value},400);dialog.showModal()}#isFull=false}
